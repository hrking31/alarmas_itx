...
exports.notificarTemperatura = onValueUpdated(
    ...
    async (event) => {
      ...
      const alto = umbralData[salaId] ?? umbralData.alto;

      if (!botToken || !receptores || typeof alto !== "number") return null; // Si no hay Telegram, umbral configurado, salimos
...
exports.verificarConexionSensores = onSchedule(
    ...
    async () => {
      ...
      const MARGEN_TIEMPO = 5 * 60 * 1000; // 5 minutos: evita notificar caídas cortas de wifi
      ...
      const updates = {};
      const promesasTelegram = []; // Para almacenar las promesas de envío de Telegram
